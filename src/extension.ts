...
export function activate(context: vscode.ExtensionContext) {
	...
	// --- 8.7 定义跳转 Provider (Go to Definition) ---
	const definitionProvider = vscode.languages.registerDefinitionProvider('riscv', {
		provideDefinition(document, position) {
			...
			if (match) {
				...
				// 定位到冒号前的标签名起始位置
				const labelStart = match[0].indexOf(word);
				const targetPos = new vscode.Position(startPos.line, labelStart);
